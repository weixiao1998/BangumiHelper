...
一个现代化的追番助手网站，支持多数据源、多用户、过滤规则与 RSS 自动下载。

## ✨ 功能特性

- 📺 **番剧日历** - 按星期展示当季番剧列表，可选年份+季度浏览
- 🔍 **多数据源（插件化）** - 当前内置蜜柑计划数据源
- 🔔 **RSS 自动下载** - 服务端生成过滤后的订阅 feed，交给你的下载器拉取（无需暴露下载器）
- 🔗 **多种下载方式** - 磁力链接、种子文件、RSS 订阅
- 👥 **多用户系统** - 用户注册登录，数据隔离
- 🎯 **智能过滤** - 支持关键词、字幕组、语言、集数范围、正则过滤
...
服务端只负责**生成 feed**，由你自己的下载器定时拉取——因此下载器在 NAT 之后也能工作，
不需要把它暴露到公网。

1. 在「我的订阅」卡片点「RSS」，复制订阅链接（或一键重新生成）
2. 在 qBittorrent / Transmission 中添加 RSS 订阅，粘贴该链接
3. 保存路径、分类、做种规则等在**下载器的 RSS 规则里**设置
4. 过滤（关键词 / 字幕组 / 语言 / 集数 / 正则）已由服务端完成，客户端不必再配关键词

> feed 默认只输出最近 **60 天**或最近 **100 条**（取先到者），避免首次添加时把历史全集灌进下载器。
> 可用 `?days=` / `?limit=` 覆盖，`0` 表示不限。暂停的订阅不会出现在 feed 中（单订阅 feed 返回 404）。

### 下载单集

1. 进入番剧详情页，选择字幕组
2. 「复制磁力」交给本机下载器，或「下载种子」获取 `.torrent` 文件
